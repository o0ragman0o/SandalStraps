...
const ensContract = (new web3.eth.contract(ensContractAbi)).at(registryAddresses[web3.version.network]);


const ens = new Lux({
  contract: (new web3.eth.contract(ensContractAbi)).at( registryAddresses[web3.version.network] ),
  ensLoading: false,
  ensAvailable: false,
  getAddr(name, callback) {
    this.ensLoading = true;
    let resolverContract = new web3.eth.contract(resolverContractAbi);
    let node = this.namehash(name);
    this.contract
      .resolver(node)
      // .call()
      .then(function(resolverAddress) {
        if (resolverAddress != 0) {
          resolverContract.address = resolverAddress;
          resolverCOntract
            .addr(node)
            // .call()
  ...
  getName(address, callback) {
    this.ensLoading = true;
    let resolverContract = new web3.eth.contract(resolverContractAbi);
    let node = this.namehash(
      address.toLowerCase().replace("0x", "") + ".addr.reverse"
    );

    this.contract.resolver(node).call(function(error, resolverAddress) {
      if (error) {
      ...
      if (resolverAddress != 0) {
        ...
        resolverContract.address = resolverAddress;
        resolverContract.name(node, function(error, result) {
...
// ensContract.resolver(namehash('ens'));
